...
class DreamLLMDataset(Dataset):
    def __init__(
        ...
        vision_encoder_processor: Callable,
        stable_diffusion_head_processor: Callable,
        vision_embedding_len: int,
        ...
        self.vision_encoder_processor = vision_encoder_processor
        self.stable_diffusion_head_processor = stable_diffusion_head_processor
        self.vision_embedding_len = vision_embedding_len
    ...
    def _merge_text_list(self, text_list, matched_text_index):
        ...
        for index in matched_text_index:
            text_to_join = [text for text in text_list[prev_index : index + 1] if text is not None]
            new_text_list.append(" ".join(text_to_join))
            prev_index = index + 1
        if prev_index != len(text_list):
            text_to_join = [text for text in text_list[prev_index:] if text is not None]
            new_text_list.append(" ".join(text_to_join))
    ...
    def _image_ids(self):
        ...
        image_ids = [image_patch_id] * self.vision_embedding_len
    ...
    def _image_strs(self):
        image_strs = "{}{}{}".format(DEFAULT_IMAGE_START_TOKEN, 
                                     DEFAULT_IMAGE_PATCH_TOKEN * self.vision_embedding_len,
                                     DEFAULT_IMAGE_END_TOKEN)
        return image_strs
    ...
    def __getitem__(self, index):
        ...
        if sample.dataset_type == DatasetType.ImageImagePair:
            ...
            add_time_ids = None
            try:
                images = [self.vision_encoder_processor.preprocess(image, return_tensors="pt")["pixel_values"][0]]
            except:
                images = []
            ...
            if self.comprehension_only or (not self.creation_only and not self.comprehension_only and prob >= 0.5):
                ...
                input_ids = [self.tokenizer.bos_token_id] + image_ids + input_ids + [self.tokenizer.eos_token_id]
                attention_mask = [1] * len(input_ids)
                labels = copy.deepcopy(input_ids)
                ...
                input_ids = self.tokenizer(
                    text,
                    max_length=self.tokenizer.model_max_length - len(dream_ids) - 2,  # -2 for bos eos
                    truncation=True,
                ).input_ids
                # input_ids = [self.tokenizer.bos_token_id] + input_ids + [self.tokenizer.eos_token_id] + dream_ids
                input_ids = [self.tokenizer.bos_token_id] + input_ids + dream_ids + [self.tokenizer.eos_token_id]
                attention_mask = [1] * len(input_ids)
            ...
            if self.skip_long_sample and (len(input_ids) > 512):
                logger.warning(f"long caption sample with token length: {len(input_ids)} meeted, skipped")
                return self.__getitem__((index + 1) % len(self))
            ...
            text_list = [text.strip() for text in text_list if text is not None]
            ...
            for idx, text in enumerate(text_list):
                ...
                cur_input_ids_no_bos_eos = results.input_ids        # Qwen2 tokenizer不会自动加bos
                ...
                if idx < len(image_list):
                    ...
                    try:
                        if self.comprehension_only:
                            images.append(self.vision_encoder_processor.preprocess(image_list[idx], return_tensors="pt")["pixel_values"][0])
                        elif self.creation_only:
            ...
            assert self.conv_template.sep_style == SeparatorStyle.MPT, "Only support `MPT` now."
            ...
            for idx in instruction_image_idx:
                ...
                images.append(self.vision_encoder_processor.preprocess(cur_compre_image, return_tensors="pt")["pixel_values"][0])
            ...
            for idx, text in enumerate(text_list):
                results = self.tokenizer(text)
                # cur_input_ids_no_bos_eos = results.input_ids[1:]      # Qwen2 tokenizer不会自动加bos
                cur_input_ids_no_bos_eos = results.input_ids
                input_ids = input_ids + cur_input_ids_no_bos_eos
                ...
                if idx < len(image_list):
                    ...
                    if self.use_rgba:
                        ...
                        cur_image = image_list[idx] if image_list[idx].mode == 'RGBA' else image_list[idx].convert('RGBA')
                        cur_create_image = Image.alpha_composite(Image.new('RGBA', cur_image.size, (255, 255, 255, 255)), cur_image).convert('RGB')
            ...
            assert self.conv_template is not None, "The `conv_template` should be specified."
            # logger.debug(f"conv_template: {self.conv_template}")
            ...
            sep_assistant = self.conv_template.sep + self.conv_template.roles[1] + "\n"   # <|im_start|>assistant\n
            sep_user = self.conv_template.sep + self.conv_template.roles[0] + "\n"   # <|im_start|>user\n
            
            sep = self.conv_template.sep    # <|im_start|>
            conv = prompt.split(sep)        # 按照 <|im_start|> 分割
            cur_len = 0
            for i, turn in enumerate(conv):
                if turn == "":
                    continue
                if turn.startswith(("system", "user")):
                    turn = sep + turn
                    turn_len = len(self.tokenizer(turn).input_ids)
                    labels[cur_len : cur_len + turn_len] = [IGNORE_INDEX] * turn_len
                    cur_len += turn_len
                elif turn.startswith("assistant"):  # ignore "<|im_start|>assistant\n"
                    turn = sep + turn.replace('<image>', '<dream>')
                    generation_prompt_len = len(self.tokenizer(sep_assistant).input_ids)
                    labels[cur_len : cur_len + generation_prompt_len] = [IGNORE_INDEX] * generation_prompt_len
                    cur_len += len(self.tokenizer(turn).input_ids)
                else:
                    raise ValueError(f"Unknown startwith turn: {turn}")

            # FIXME: This is a hack to walk around the tokenizer bug.
            if cur_len != len(input_ids):
                logger.debug(f"A sample is going wroing with length, ignore it! cur_len: {cur_len}, len(input_ids): {len(input_ids)}")
            ...
            if len(input_ids) > self.tokenizer.model_max_length:
                logger.warning(f"long samples with length of conversation: {len(input_ids)}, skipped")
                return self.__getitem__((index + 1) % len(self))
            ...
            for id in input_ids:
                if id == image_start_id:
                    images.append(
                        self.vision_encoder_processor.preprocess(
        ...
        # logger.debug(f"input type: {sample.dataset_type}, len_input: {len(input_ids)}")
        return_dict = {
