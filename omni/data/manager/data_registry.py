...
class DataRegistry:
    ...
    def __call__(
        ...
        if approx_sizes:
            assert len(datasets) == len(approx_sizes), "The length of `datasets` and `approx_sizes` should be the same."
        if shard_lists:
            assert len(datasets) == len(shard_lists), "The length of `datasets` and `shard_lists` should be the same."
        ...
        for idx, name in enumerate(datasets):
            try:
                ...
                if approx_sizes:
                    info.approx_size = approx_sizes[idx]
                if shard_lists:
                    info.shard_list = megfile.smart_glob(shard_lists[idx])
