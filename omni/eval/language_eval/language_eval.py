...
def load(ckpt_dir: str, num_gpus: int, max_seq_len=1024, max_batch_size=32, add_special_pattern=False):
    ...
    model = AutoModelForCausalLM.from_pretrained(ckpt_dir, tokenizer=tokenizer, low_cpu_mem_usage=True, torch_dtype=torch.float32)
...
cheese =>""",
    ...
    results = model.generate(prompts, max_gen_len=256, max_seq_len=max_seq_len, temperature=temperature, top_p=top_p)

    for result in results:
        print(result)
        print("\n==================================\n")

main("work_dirs/sft_llava_20241113")