...
from transformers import CLIPImageProcessor, CLIPModel, CLIPProcessor, CLIPVisionModel, AutoTokenizer, LlamaTokenizer
...
def dreamllm_ddp_sample_coco(
    ...
    for step in range(steps):
        ...
        for _info in info:
            ...
            prompt.append("<s>USER: Please generate a photo of " + random.choice(captions[:index]) + "ASSISTANT: ")
        ...
        images.extend(
            model.stable_diffusion_pipeline(
                ...
                # negative_prompt=['<|endoftext|>'] * len(prompt)
            )
        )
