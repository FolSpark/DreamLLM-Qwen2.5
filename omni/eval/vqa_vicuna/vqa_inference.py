...
from omni.utils.conversation import KeywordsStoppingCriteria, SeparatorStyle, conv_vicuna_v1_1 as default_conversation
...
def generate(args, tokenizer, conv, image_tensor, model):
    ...
    keywords = [stop_str, DEFAULT_DREAM_START_TOKEN]
    stopping_criteria = KeywordsStoppingCriteria(keywords, tokenizer, input_ids, skip_special_tokens=False, dream_start_id=tokenizer.convert_tokens_to_ids([DEFAULT_DREAM_START_TOKEN])[0])
    ...
    with torch.autocast("cuda", dtype=torch.bfloat16):
        ...
        elif str(args.beamsearch) == "True_short":
            output_ids = model.generate(
                input_ids,
                images=image_tensor.unsqueeze(0).half().cuda(),
                num_beams=5,
...
def main(args):
    ...
    path = "/home/quyd/pretrained-models/clip-vit-large-patch14"
    ...
    # image_processor = model.model.vision_encoder.processor
    # image_token_len = model.model.vision_encoder.embed_len
