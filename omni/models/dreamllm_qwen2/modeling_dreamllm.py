...
from omni.models.dreamllm_qwen2.modeling_plugins import PipelineImageType
...
class DreamLLMForCausalMLM(DreamLLMPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        # logger.debug(f"input_ids.shape: {input_ids.shape if input_ids is not None else None}")
        # logger.debug(input_ids)
        # logger.debug(f"labels.shape: {labels.shape if labels is not None else None}")
        # logger.debug(labels)
        # logger.debug(f"images.shape: {images.shape if images is not None else None}")
        # logger.debug(f"inputs_embeds.shape: {inputs_embeds.shape if inputs_embeds is not None else None}")
        ...
        if self.training and images_dm is not None:
            ...
            # if images is not None:
            #     logger.debug(f"input_ids.shape: {input_ids.shape}, images.shape: {images.shape}, images_dm: {images_dm}")
            # else:
            #     logger.debug(f"input_ids.shape: {input_ids.shape}, images: {images}, images_dm: {images_dm}")
