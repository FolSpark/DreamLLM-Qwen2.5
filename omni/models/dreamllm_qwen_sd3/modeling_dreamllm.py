...
from omni.models.dreamllm_qwen_sd3.modeling_plugins import PipelineImageType
from omni.models.dreamllm_qwen_sd3.tokenization_dreamllm import (
    DEFAULT_BOS_TOKEN,
...
# Copy from modeling_qwen2.py: start
...
class DreamLLMModel(DreamLLMPreTrainedModel):
    ...
    def forward(
        ...
        # logger.debug(f"images.shape: {images.shape}")   # [image总数], 3, 224, 224
        # HACK: not elegent, how can we auto select `clip_vision_embedding`
        image_features = self.clip_vision_embedding(images)
        # logger.debug(f"image_features.shape: {image_features.shape}") # [image总数], 256, hidden_size
        ...
        if inputs_embeds is not None:
            if images is not None:
                ...
                new_input_embeds = []
                cur_image_idx = 0
                # logger.debug(f"inputs_embeds.shape: {inputs_embeds.shape}") # bs, seq_len, hidden_size
                # logger.debug(f"input_ids.shape: {input_ids.shape}") # bs, seq_len
...
class DreamLLMForCausalMLM(DreamLLMPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        if self.training and images_dm is not None:
            ...
            if self.stable_diffusion3_head.drop_prob is not None:
            ...
            if isinstance(images_dm, dict):
                for key, cur_images_dm in images_dm.items():
                    ...
                    if cur_bs > 0:
                        for diffusion_idx in range(-(-cur_images.shape[0] // cur_bs)):
                            ...
                            vm_loss += cur_mini_bs * self.stable_diffusion3_head(cur_images[start_idx: end_idx],
                                                                                cur_encoder_hidden_states[start_idx: end_idx],
                                                                                cur_u_encoder_hidden_states and cur_u_encoder_hidden_states[start_idx: end_idx])
                    else:
                        vm_loss += cur_images.shape[0] * self.stable_diffusion3_head(cur_images, cur_encoder_hidden_states, cur_u_encoder_hidden_states)
                ...
                if self.diffusion_bs > 0:
                    for diffusion_idx in range(-(-images_dm_len // self.diffusion_bs)):
                        ...
                        vm_loss += cur_mini_bs * self.stable_diffusion3_head(images_dm[start_idx: end_idx], 
                                                                            encoder_hidden_states[start_idx: end_idx],
                                                                            u_encoder_hidden_states and u_encoder_hidden_states[start_idx: end_idx])
                    vm_loss /= images_dm_len
                else:
                    vm_loss = self.stable_diffusion3_head(images_dm, encoder_hidden_states, u_encoder_hidden_states)
        elif self.training:
            ...
            vm_loss = self.stable_diffusion3_head(images_dm, None, None, self.model.dream_embedding())
    ...
    def encode_prompt(
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            if negative_prompt is None:
                uncond_tokens = [tokenizer.pad_token] * batch_size
    ...
    @torch.no_grad()
    def stable_diffusion3_pipeline(
        self,
        tokenizer: PreTrainedTokenizerBase,
        ...
        return self.stable_diffusion3_head.pipeline(
            height=height,
            width=width,
