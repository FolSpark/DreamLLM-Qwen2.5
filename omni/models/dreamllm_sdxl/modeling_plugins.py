...
class StableDiffusionXLHead(StableDiffusionHead):
    def __init__(
        ...
        self.global_projector = build_projector(projector_cfg, in_hidden_size=embed_hidden_size, out_hidden_size=self.global_condition_hidden_size, bias=False)
        if not self.global_projector.load_model(projector_name_or_path):
            self._init_weights(self.global_projector)

        if pretrained_model_name_or_path is not None and pretrained_model_name_or_path != 'None':
    ...
    def forward(
        ...
        if is_dummy:
            ...
            if self.freeze_unet:
                ...
                dummy_noisy_latents = torch.zeros(1, 4, 128, 128, device=self.device, dtype=self.dtype)
                ...
                return (0.0 * dummy_model_pred).sum() + (0.0 * dummy_global_image_features).sum() + (0.0 * dream_embeddings).sum()
        ...
        unet_added_conditions.update({"text_embeds": global_encoder_hidden_states})

        # Predict the noise residual and compute loss
        model_pred = self.unet(noisy_latents, timesteps, encoder_hidden_states, added_cond_kwargs=unet_added_conditions).sample
