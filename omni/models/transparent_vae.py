...
class TransparentVAE(AutoencoderKL):
    ...
    def set_transparent_encoder(self, sd, encoder_alpha=300.0):
        ...
        # model.eval()
        self.transparent_encoder = model
    ...
    def transparent_encode(self, x: torch.Tensor, return_dict: bool = True, use_offset: bool = True):
        rgb_bchw_01 = x[:, :3, :, :]
        a_bchw_01 = x[:, 3:, :, :]
        vae_feed = rgb_bchw_01 * (a_bchw_01 * 0.5 + 0.5)
        latent_dist = super().encode(vae_feed).latent_dist
        offset_feed = torch.cat([a_bchw_01, rgb_bchw_01], dim=1)
        offset = self.transparent_encoder(offset_feed) * self.encoder_alpha
        if use_offset:
    ...
    def transparent_decode(self, z: torch.Tensor, return_dict: bool = True, generator=None) -> Union[DecoderOutput, torch.Tensor]:
