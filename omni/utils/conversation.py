...
class KeywordsStoppingCriteria(StoppingCriteria):
    def __init__(self, keywords, tokenizer, input_ids, skip_special_tokens=True, dream_start_id=50256):
        self.keywords = keywords
        self.tokenizer = tokenizer
        self.start_len = None
        self.input_ids = input_ids
        self.skip_special_tokens = skip_special_tokens
        self.dream_start_id = dream_start_id

    def __call__(self, output_ids: torch.LongTensor, scores: torch.FloatTensor, **kwargs) -> bool:
        if torch.any(output_ids == self.dream_start_id).item():
            return True
        if self.start_len is None:
            self.start_len = self.input_ids.shape[1]
        else:
            outputs = self.tokenizer.batch_decode(output_ids[:, self.start_len :], skip_special_tokens=self.skip_special_tokens)[0]
...
qwen_conv_mpt_multimodal = Conversation(
    system="""<|im_start|>system
...
default_conversation = qwen_conv_mpt_multimodal
conv_templates = {
